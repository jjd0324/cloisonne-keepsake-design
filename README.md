...
这是一个支持**参考图、纯文字、参考图加文字**三种入口的 Agent Skill：先建立人物、宠物、风景、建筑或纪念主题的内容规格，再独立选择风格与输出形式，最后生成并验收图片。
...
对外展示名已升级为“多风格图像与纪念品设计”。技术标识、仓库名和调用命令仍保留 `cloisonne-keepsake-design`，避免已有安装与链接失效。

本 Skill 只交付视觉概念：不提供可直接投产的 CAD、开模图、工程尺寸、报价或生产证明；数字艺术图也不代表真实手绘原作已经制作。

## 使用流程

1. **提供内容**：可上传参考图、只写文字描述，或用图片加文字补充；只有两者都缺失时才会追问。
2. **建立内容规格**：图片路线锁定主体与构图不变量；纯文字路线锁定用户写明的主体、数量、动作、场景、颜色和氛围；混合路线以图片为事实锚点、文字为修改要求。
3. **选择风格**：掐丝珐琅，或极简纸感丙烯。
4. **选择输出形式**：实体纪念品，或单幅艺术图。
5. **生成与验收**：按所选分支生成，并分别检查输入一致性、风格辨识和输出结构。
...
> 使用 $cloisonne-keepsake-design，不上传图片：生成一枚圆形掐丝珐琅冰箱贴，主题是一只戴红围巾的黑猫坐在雪夜窗边。

> 使用 $cloisonne-keepsake-design，以这张建筑照片为构图参考，并按文字要求改成秋日配色的极简纸感丙烯艺术图。

如果当前环境不能读取图片或没有图片生成工具，Skill 只交付已标注状态的提示词、设计规格与验收规则，不假装已经生成图片。

## 模板画廊

以下是可公开复用的通用 AI 概念预览。它们不包含、也不复刻任何用户原始照片；每个模板均可由参考图、文字描述或两者组合驱动，并包含风格、输出结构、可复制提示词和验收点。
